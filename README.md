...
Please note that if you are using the `shell` option with a functional configuration, _lint-staged_ cannot escape the filenames since the function is supposed to generate the entire command string. The default `escapeArg` method can be required for use:

```js
// .lintstagedrc.js
const { escapeArg } = require('lint-staged')

module.exports = {
  '**/*.js?(x)': (filenames) => `eslint ${filenames.map(escapeArg).join(' ')}`,
}
```
